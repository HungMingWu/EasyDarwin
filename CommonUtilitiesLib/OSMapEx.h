...
#define sLastingTimeOneYear (int64_t)60*1000*1000*60*24*365//持续时间1年

//SesssionID Map
struct strMapData
{
	char m_AllExist;
	int64_t m_LastingTime;
	strMapData() :m_AllExist(0), m_LastingTime(0) {}
	strMapData(int64_t LastingTime, char AllExist = 0) :m_AllExist(AllExist), m_LastingTime(LastingTime) {}
};
...
public:
	...
	bool Insert(const string& strSessionID, int64_t lastingTime = sLastingTimeOneYear);//插入，线程安全，返回true成功插入，否则重复插入,用于一些长期存在的SessionID
	//bool Delete(string&strSessionID);//删除,线程安全，成功删除返回true,否则false

	string GererateAndInsert(int64_t lastingTime = sLastingTimeOneMin);//生成一个唯一的sessionID并插入到map中，返回sessionid,线程安全，lastingTime为微秒单位
